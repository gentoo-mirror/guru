...
inherit edo cargo flag-o-matic multiprocessing
...
src_test() {
	# Use release profile to avoid ring crate linking issues in debug builds
	# Exclude e2e tests which require network access to external repositories
	# Filter out LTO flags as ring crate's assembly is incompatible with LTO
	local -x CARGO_BUILD_JOBS=${CARGO_BUILD_JOBS:-$(makeopts_jobs)}
	filter-lto
	edo cargo nextest run --release --workspace --no-fail-fast -E 'not test(/e2e/)'
}
