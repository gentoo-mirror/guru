...
src_install() {
	if use gtk4; then
		local GNOME_SSH_ASKPASS="gnome-ssh-askpass4"
	else
		local GNOME_SSH_ASKPASS="gnome-ssh-askpass3"
	fi

	dobin "${GNOME_SSH_ASKPASS}";

	newenvd - 99gnome_ssh_askpass <<-EOF
		export SSH_ASKPASS='${EPREFIX}/usr/bin/${GNOME_SSH_ASKPASS}'
	EOF
}
