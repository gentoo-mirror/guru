...
pkg_postinst() {
	# Enabling is a one-time manual step that touches nothing under
	# /etc/portage/, so only show the setup guide on a fresh install.
	# Repeating it on every upgrade would just be noise for users who
	# have already wired it up.
